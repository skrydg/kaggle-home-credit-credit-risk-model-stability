...
class ProcessCreditBureaua2TableStep:
    def __init__(self):
        ...
        self.active_contracts_columns = ["collater_typofvalofguarant_298M", "collater_valueofguarantee_1124L", "collaterals_typeofguarante_669M", "pmts_dpd_1073P", "pmts_month_706T", "pmts_overdue_1140A", "pmts_year_1139T", "subjectroles_name_838M"]
    ...
    def process(self, dataset, columns_info):
        ...
        dataset.set("credit_bureau_a_active_contracts_2", table.filter(mask)[self.active_contracts_columns])

        # close contracts
        column = "collater_valueofguarantee_876L"
        mask = table[column].is_not_null()
        assert(column in self.close_contracts_columns)
        dataset.set("credit_bureau_a_close_contracts_2", table.filter(mask)[self.close_contracts_columns])

        # existed contracts
        column = "pmts_overdue_1140A"
        mask = table[column].is_not_null()
        assert(column in self.existed_contracts_columns)
        dataset.set("credit_bureau_a_existed_contracts_2", table.filter(mask)[self.existed_contracts_columns])
